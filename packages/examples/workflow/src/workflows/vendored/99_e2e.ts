...
export class Sensor {
   ...
   *  NOTE: getter steps are not supported by @restatedev/workflow's SWC plugin
   *  yet. We stub the getter so the file compiles; the getterStepWorkflow test
   *  is skipped in e2e.test.ts. */
  get reading(): Promise<number> {
    return Promise.resolve(this.baseValue * this.multiplier);
  }
