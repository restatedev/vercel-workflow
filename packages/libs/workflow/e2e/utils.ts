...
/**
 * Configures the world for the Restate test runner. The runner process needs
 * its own world handle so it can call `world.runs.get(...)` from inside tests.
 * `WORKFLOW_TARGET_WORLD=@restatedev/workflow/world` is set by scripts/e2e.sh.
 */
export function setupWorld(_deploymentUrl: string): void {
  // No-op: Restate world is constructed lazily by getWorld() based on
  // WORKFLOW_TARGET_WORLD + RESTATE_INGRESS / RESTATE_ADMIN_URL env vars.
}
